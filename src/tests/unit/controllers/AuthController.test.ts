...
describe('AuthController.createAccount', () => {
    ...
    it('should return a 409 status and an error message if the email es already regsitered', async () => {
    ...
    it('should register new user and return a success message', async () => {
        const req = createRequest({
            method: 'POST',
            url: '/api/auth',
            body: { email: 'test@test.com', password: 'password', name: 'Test Namer' },
        });
        const res = createResponse();
...
describe('AuthController.login', () => {
    ...
    it('should return 403 if user is not confirmed', async () => {
        ...
        expect(data).toHaveProperty('error', 'La cuenta no está confirmada');
    })

    it('should return 401 if user is the password is incorrect', async () => {
        const userMock = {
            id: 1,
            email: 'test@test.com',
            password: 'password',
        ...
        expect(data).toHaveProperty('error', 'La contraseña no es correcta');
