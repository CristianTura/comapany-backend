...
describe('budget Middleware - validateBudgetExists', () => {
    it('should handle non-existent budget', async () => {
        ...
        expect(data).toEqual({error: 'No se encontró el presupuesto'})
    ...
    it('should proceed to next if budget existent', async () => {
...
describe('budget Middleware - hasAccess', () => {
    it('should restrict if user does not have permission', async () => {
        const req = createRequest({
            budget: { userId: 1 },
            user: { id: 2 }
        })
    ...
    it('should proceed to next if user has permission', async () => {
        const req = createRequest({
            budget: { userId: 1 },
            user: { id: 1 }
        })
